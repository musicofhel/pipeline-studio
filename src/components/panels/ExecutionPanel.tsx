    ...
    <div className="flex h-full flex-col">
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-0.5 p-2">
          {nodes
            ...
            .sort((a, b) => {
              const order: Record<string, number> = { success: 0, running: 1, error: 2, skipped: 3, blocked: 4 }
              return (order[a.data.execution!.status] ?? 5) - (order[b.data.execution!.status] ?? 5)
            })
            .map((n) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    {exec.status === 'success' && <CheckCircle size={12} className="text-green-400" />}
                    {exec.status === 'error' && <XCircle size={12} className="text-red-400" />}
                    {exec.status === 'running' && <div className="h-3 w-3 animate-spin rounded-full border-2 border-yellow-400 border-t-transparent" />}
                    {exec.status === 'skipped' && <div className="h-3 w-3 rounded-full bg-zinc-600" />}
                    <span className="text-zinc-300">{def?.label ?? n.type}</span>
                  </div>
                  <div className="flex items-center gap-2 text-zinc-500">
                    {exec.latencyMs !== undefined && (
                      <span>{exec.latencyMs < 1000 ? `${exec.latencyMs.toFixed(0)}ms` : `${(exec.latencyMs / 1000).toFixed(2)}s`}</span>
                    )}
                    {exec.error && <span className="max-w-32 truncate text-red-400">{exec.error}</span>}
                  </div>
                </div>
              )
            })}
      ...
      {latestRun?.response && (
        <div className="border-t border-zinc-700 p-3">
          <h4 className="mb-1 text-xs font-medium text-zinc-400">Response</h4>
          <p className="text-xs leading-relaxed text-zinc-300">{latestRun.response}</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-4 border-t border-zinc-700 px-3 py-1.5 text-[10px] text-zinc-500">
        ...
        {runningNodes.length > 0 && <span className="text-yellow-400">{runningNodes.length} running</span>}
        {errorNodes.length > 0 && <span className="text-red-400">{errorNodes.length} errors</span>}
        {skippedNodes.length > 0 && <span>{skippedNodes.length} skipped</span>}
      </div>
    </div>
  )
}
